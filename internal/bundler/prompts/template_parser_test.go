...
func TestParsePrompt(t *testing.T) {
	...
	result := ParsePrompt(prompt)

	// Test system template
	require.Len(t, result.System.Variables, 2)

	// Test prompt template
	require.Len(t, result.Prompt.Variables, 1)
}

func TestParseTemplateVariables(t *testing.T) {
	t.Run("empty template", func(t *testing.T) {
		result := ParseTemplateVariables("")
		assert.Empty(t, result)
	})

	t.Run("no variables", func(t *testing.T) {
		result := ParseTemplateVariables("You are a helpful assistant.")
		assert.Empty(t, result)
	})

	t.Run("legacy syntax", func(t *testing.T) {
		result := ParseTemplateVariables("You are a {{role}} assistant.")
		assert.Equal(t, []string{"role"}, result)
	})

	t.Run("new syntax", func(t *testing.T) {
		result := ParseTemplateVariables("You are a {role:assistant} specializing in {!domain}.")
		assert.ElementsMatch(t, []string{"role", "domain"}, result)
	})

	t.Run("mixed syntax", func(t *testing.T) {
		result := ParseTemplateVariables("You are a {{role}} {domain:AI} {!specialization} assistant.")
		assert.ElementsMatch(t, []string{"role", "domain", "specialization"}, result)
	})

	t.Run("duplicate variables", func(t *testing.T) {
		result := ParseTemplateVariables("You are a {role:assistant} {role:helper}.")
		assert.Equal(t, []string{"role"}, result)
	})
}
