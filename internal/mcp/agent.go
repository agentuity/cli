...
func init() {
	register(func(c MCPContext) error {
		return c.Server.RegisterTool("create_agent", "this is a tool for creating an Agent using the agentuity cloud platform", func(ctx context.Context, args CreateAgentArguments) (*mcp_golang.ToolResponse, error) {
			...
			result, err := execCommand(ctx, c.ProjectDir, "agent", "create", args.Name, args.Description, args.AuthType)
			if err != nil {
				return nil, err
			}
	...
	register(func(c MCPContext) error {
		return c.Server.RegisterTool("list_agents", "this is a tool for listing information about the Agents in this agentuity cloud platform project", func(ctx context.Context, args NoArguments) (*mcp_golang.ToolResponse, error) {
			if resp := ensureLoggedIn(c); resp != nil {
				return resp, nil
			}
			if resp := ensureProject(c); resp != nil {
			...
			result, err := execCommand(ctx, c.ProjectDir, "agent", "list")
			if err != nil {
				return nil, err
			}
			return mcp_golang.NewToolResponse(mcp_golang.NewTextContent(result)), nil
